...
void kalindi_init_pixel_pll(struct radeon_device * rdev, uint8_t pll_id)
{
	...
	kalindi_set_pixel_clock(rdev, pll_id, &pll_set, crtc_id, px_clk, flags,
			      encoder_mode);
}

static uint32_t kalindi_get_vga_ctl_offset(uint8_t block_id)
{
			   ...
			   uint32_t px_clk, uint32_t fags, uint8_t encoder_mode)
{
	...
	/* Should have called kalindi_set_disp_eng_pll you son of a bitch! */
	...
	if (!(fags & PIXEL_CLOCK_V6_MISC_FORCE_PROG_PPLL)) {
		if (crtc_id <= 3) {
			...
			off = kalindi_get_vga_ctl_offset(crtc_id);
...
static int kalindi_program_ext_pll(struct radeon_device *rdev, uint8_t pll,
			   ...
			   uint16_t pcie_ss_percent)
{
	...
	pll = kalindi_compute_engine_pll(vco_freq_mhz, &desired_clock);
	desired_clock /= 10;
	if (kalindi_program_ext_pll(rdev, pll, DIV_REFCLOCK) < 0) {
	...
	pll = kalindi_compute_engine_pll(vco_freq_mhz, &desired_clock);
	desired_clock /= 10;
	if (kalindi_program_ext_pll(rdev, pll, DIV_ENGINE) < 0) {
