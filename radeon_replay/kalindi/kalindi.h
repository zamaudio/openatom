...
/* backlight.c */
void kalindi_backlight_init(struct radeon_device *rdev, bool controls_edp);
void kalindi_backlight_control(struct radeon_device *rdev,
			     uint16_t bl_pwm_freq_hz, uint8_t bl_level);
void kalindi_backlight_blon(struct radeon_device *rdev);
void kalindi_backlight_bloff(struct radeon_device *rdev);

/* crtc.c */
int kalindi_enable_crtc(struct radeon_device *rdev, uint8_t crtc_id, bool enable);
int kalindi_update_crtc_x2_buf(struct radeon_device *rdev,
			     uint8_t crtc_id, bool enable);
int kalindi_blank_crtc(struct radeon_device *rdev, uint8_t crtc_id, bool enable);
void kalindi_set_crtc_dtd_timing(struct radeon_device *rdev, uint8_t crtc_id,
			       struct drm_display_mode *mode);
void kalindi_overscan_setup(struct radeon_device *rdev, uint8_t crtc_id,
...
/* encoder.c */
void kalindi_encoder_setup_dp(struct radeon_device *rdev, uint8_t id,
...
void kalindi_encoder_link_training_start(struct radeon_device *rdev,
				       uint8_t encoder_id);
void kalindi_encoder_link_training_pattern(struct radeon_device *rdev,
					 uint8_t encoder_id, uint8_t pattern);
void kalindi_encoder_link_training_finish(struct radeon_device *rdev,
					uint8_t encoder_id);
void kalindi_set_encoder_crtc_source(struct radeon_device *rdev,
...
/* pll.c */
uint8_t kalindi_compute_engine_pll(uint32_t vco_freq_mhz, uint32_t *clock_khz);
void kalindi_disable_pll_ss(struct radeon_device *rdev, uint8_t pll_id);
void kalindi_enable_pll_ss(struct radeon_device *rdev, uint8_t pll_id,
			 uint16_t step, uint16_t ds_frac, uint8_t fbdiv,
			 uint8_t nfrac, bool downspread);
void kalindi_set_pixel_clock(struct radeon_device *rdev, enum pll_ids pll_id,
			   struct pll_settings *pll_divs, uint8_t crtc_id,
			   uint32_t px_clk, uint32_t fags, uint8_t encoder_mode);
void kalindi_init_pixel_pll(struct radeon_device * rdev, uint8_t pll_id);
int kalindi_set_disp_eng_pll(struct radeon_device *rdev, uint32_t vco_freq_mhz,
