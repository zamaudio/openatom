			    ...
			    uint16_t h, uint16_t w, uint16_t fbpitch)
{
 ...
 do_enable_shit:
	...
	if (enable == 0) {
		kalindi_disable_grph_srfc(rdev, surf);
		return;
	}
...
//                Table update indicator 0
//
void kalindi_fuck_my_lute(struct radeon_device *rdev, uint8_t lute_id)
{
...
void kalindi_mcleanup(struct radeon_device *rdev, uint16_t start_8k, uint16_t size_8k)
{
	uint8_t saviour;
