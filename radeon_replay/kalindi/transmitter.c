...
#define REG_ENC_CH_MAPPING		0x6638
...
//	uint16_t offset_next;
	...
	uint16_t lane3;
		...
		.sym_clk_max = 0xffff,
		.dig_mode = TRANSMITTER_MODE_HDMI,
		.phy_mask = 0x0e,
		.lane0 = 0x2a00,
		.lane1 = 0x2a00,
		.lane2 = 0x2a00,
...
static void fucking_smu_message(struct radeon_device *rdev, uint32_t msg)
{
	...
	radeon_write(rdev, SMC_MESSAGE_0, msg);
	//   00db: 3c258c0000        COMP   reg[008c]  [...X]  <-  00
	//   00e0: 44db00            JUMP_Equal  00db
	int timeout = 1000;
	while ((radeon_read(rdev, SMC_RESP_0) & 0xff) == 0) {
		udelay(1);
		if (!--timeout) {
				  ...
				  uint8_t par3, uint8_t clk_id)
{
	...
	radeon_write(rdev, TN_SMC_IND_INDEX_0, 0x0001f610);
	//   0099: 0300408100        MOVE   WS_QUOT/LOW32 [XXXX]  <-  reg[0081]  [XXXX]
	quot = radeon_read(rdev, TN_SMC_IND_DATA_0);
	...
	radeon_write(rdev, TN_SMC_IND_DATA_0, quot);
				     ...
				     uint8_t par1, uint8_t smu_msg)
{
	...
	if (radeon_read(rdev, 0x173c) & ATOM_S6_ACC_MODE) {
		//   0011: 4a25471804        TEST   reg[1847]  [...X]  <-  04
		//   0016: 492e00            JUMP_NotEqual  002e
		if (radeon_read(rdev, 0x611c) & 0x04)
			return;
	}
	...
	radeon_write(rdev, TN_SMC_IND_INDEX_0, smu_reg_idx);
	if (smu_mask == 0xffffffff)
		radeon_write(rdev, TN_SMC_IND_DATA_0, smu_data);
	else
		radeon_mask(rdev, TN_SMC_IND_DATA_0, smu_mask, smu_data);
...
static void kalindi_pciep_mask(struct radeon_device *rdev, uint32_t pceipreg,
			      uint32_t clrbits, uint32_t setbits)
{
	uint32_t reg32;

	radeon_write_io(rdev, PCIEP_IDX, pceipreg);
	reg32 = radeon_read_io(rdev, PCIEP_DAT);
	reg32 &= ~clrbits;
	reg32 |= setbits;
	radeon_write_io(rdev, PCIEP_DAT, reg32);
}

static void kalindi_pciep_write(struct radeon_device *rdev, uint32_t pciepreg,
			      uint32_t val)
{
	radeon_write_io(rdev, PCIEP_IDX, pciepreg);
	radeon_write_io(rdev, PCIEP_DAT, val);
}
				       ...
				       uint8_t id, const uint16_t strenght[4])
{
	...
	kalindi_pciep_mask(rdev, base + 0 * 0x80, 0xffff, strenght[0]);
	kalindi_pciep_mask(rdev, base + 1 * 0x80, 0xffff, strenght[1]);
	kalindi_pciep_mask(rdev, base + 2 * 0x80, 0xffff, strenght[2]);
	kalindi_pciep_mask(rdev, base + 3 * 0x80, 0xffff, strenght[3]);
}
...
static uint8_t encoder_phy_to_asic_phy(enum encoder_phy ephy)
{
	static const uint8_t asic_phy_ids[] = {
		[ENCODER_UNIPHY_A] = 0,
		[ENCODER_UNIPHY_B] = 1,
		[ENCODER_UNIPHY_C] = 2,
		[ENCODER_UNIPHY_D] = 3,
	...
	return asic_phy_ids[ephy];
}

void kalindi_transmitter_enable(struct radeon_device *rdev, enum encoder_phy ephy,
			      ...
			      uint32_t xmit_flags)
{
	uint32_t off, rem, quot;
	uint8_t par3, par2, par1;
	uint32_t setbit = 0, sb2 = 0;
	uint16_t ppar[4];
	const uint8_t phy_id = encoder_phy_to_asic_phy(ephy);
	...
	if (radeon_read(rdev, 0x6618 + off) & BIT(0)) {
	...
	radeon_mask(rdev, 0x7140 + off, 0xff << 16, phy_mode << 16);
	...
	if (hpd_id != 0) {
		...
		radeon_mask(rdev, 0x7140 + off, 0xff << 24, (hpd_id - 1) << (24 + 4));
	}
	//   00f7: COMP   phy_mode  <-  05
	//   00fb: JUMP_Equal  011a
	//   00fe: COMP   phy_mode  <-  00
	//   0102: JUMP_Equal  011a
	if ((phy_mode != TRANSMITTER_MODE_DP_MST)
	   && (phy_mode != TRANSMITTER_MODE_DP)) {
		//   0105: COMP  num_lanes  <-  04
		//   0109: JUMP_BelowOrEq  0111
		if (num_lanes > 4)
			//   010c: OR     reg[1c00]  [.X..]  <-  01
			radeon_mask(rdev, 0x7000 + off, 0, 1 << 16);
		//   0111: MOVE   reg[1c8d]  [..XX]  <-  010f
		radeon_mask(rdev, 0x7208 + off, 0xffff, 0x010f);
		//   0117: JUMP   013c
	} else {
		//   011a: MOVE   phy_mode  <-  00
		phy_mode = TRANSMITTER_MODE_DP;
		//   011e: OR     cfg->ucConfig  <-  02
		xmit_flags |= XMIT_FLAG_COHERENT;
		...
		radeon_mask(rdev, 0x7308 + off, 0xff, num_lanes);
		//   0137: MOVE   reg[1c8d]  [..XX]  <-  WS_QUOT/LOW32 [..XX]
		radeon_mask(rdev, 0x7234 + off, 0xffff, quot & 0xffff);
	}
	//   013c: TEST   reg[1c51]  [...X]  <-  01
	//   0141: JUMP_Equal  0150
	if (radeon_read(rdev, 0x7144 + off) & BIT(0)) {
	...
	if (phy_mode == TRANSMITTER_MODE_DP) {
		...
		while (1) {
			...
			if (((1 << phy_id) & x92ed->phy_mask) &&
			    (phy_mode == x92ed->dig_mode) &&
			    (symbol_clk <= x92ed->sym_clk_max))
				break;
			x92ed++;
		}
...
// 		}
	...
	program_phy_drive_strength(rdev, phy_id, ppar);
	...
	const struct clk_src_params* x916d = &clk_src_param_table[phy_src_pll];
	//   0358: MOVE   WS_REGPTR [..XX]  <-  data[0004] [..XX]
	off = x916d->reg_offset;
	...
	if (quot != 3) {
		...
		if (quot & XMIT_FLAG_COHERENT)
			//   037c: MOVE   WS_REMIND/HI32 [..X.]  <-  WS_REMIND/HI32 [...X]
			rem = x916d->smells_like_a_bitmask
			      | (x916d->smells_like_a_bitmask << 8);
	}
	//   0380: MOVE   reg[1706]  [...X]  <-  WS_REMIND/HI32 [..X.]
	radeon_mask(rdev, 0x5c18 + off, 0xff, (rem >> 8) & 0xff); /* PLL WARING!!! */
	//   0385: SET_REG_BLOCK  0000
	off = 0;		/* FIXME: Of cpurse we're not sure it's what it means */
	//   0388: TEST   cfg->ucConfig  <-  01
	//   038c: JUMP_NotEqual  03e0
	if (!(xmit_flags & XMIT_FLAG_BLACKMAGIC)) {
		//   038f: MOVE   reg[0038]  [XXXX]  <-  data[0000] [XXXX]
		radeon_write(rdev, PCIEP_IDX, x916d->pciep_reg_idx);
		...
		do {
			...
			if (symbol_clk <= x960d->max_sym_clk)
				break;
	...
	if ((phy_src_pll != PPLL_2) && (phy_src_pll != PPLL_0))
 ...
 l_clusterfuck:
	...
	if (!(xmit_flags & XMIT_FLAG_COHERENT))
		...
		radeon_mask(rdev, 0x661c + off, 0x03, 0);
		//   0474: JUMP   047d
	else
		//   0477: MASK   reg[1987]  [...X]  &  fc  |  02
		radeon_mask(rdev, 0x661c + off, 0x03, 0x02);
	//   047d: OR     reg[1986]  [...X]  <-  01
	radeon_mask(rdev, 0x6618 + off, 0, 0x01);
	...
	int timeout = 1000;
	while (!(radeon_read(rdev, PCIEP_DAT) & BIT(2))) {
		if (--timeout == 0) {
	...
	if (!(xmit_flags & XMIT_FLAG_BLACKMAGIC)) {
		...
		radeon_write(rdev, PCIEP_DAT, quot);
		//   049f: SET_ATI_PORT  0003  (INDIRECT_IO_PCIE)
		//   04a2: AND    reg[2002]  [X...]  <-  f7
		kalindi_pciep_mask(rdev, 0x1222002, 0x80 << 24, 0);
		//   04a7: OR     reg[2008]  [...X]  <-  01
		kalindi_pciep_mask(rdev, 0x1222008, 0, 1);
		//   04ac: AND    reg[2008]  [...X]  <-  fe
		kalindi_pciep_mask(rdev, 0x1222008, 1, 0);
		//   04b1: OR     reg[2002]  [X...]  <-  08
		kalindi_pciep_mask(rdev, 0x1222002, 0, 0x80 << 24);
	...
	radeon_mask(rdev, 0x6634 + off, 0, 1);
	//   04ee: DELAY_MicroSec  0a
	usleep(10000);
 l_04f0:
	//   04f0: OR     reg[198a]  [..X.]  <-  10
	radeon_mask(rdev, 0x6628 + off, 0, 0x10 << 8);
...
void kalindi_transmitter_vsemph(struct radeon_device *rdev, uint8_t phy_id,
			      uint8_t lane_sel)
{
	const struct phy_par *phy = &phy_info[phy_id];
