...
void radeon_init_aux_pads(struct radeon_device* rdev)
{
	...
	for (i = 0; i < 6; i++) {
		...
		radeon_write(rdev, block + 0x20, 0x00320000);
		radeon_write(rdev, block + 0x24, 0x00001c00);
		radeon_write(rdev, block + 0x28, 0x123d1210);
	}

	for (i = 0x10; i > 0 ; i--) {
		radeon_mask(rdev, 0x65fc, 0, 0x40 << 8);
		radeon_mask(rdev, 0x65fc, BIT(14), 0);
	}
	radeon_mask(rdev, 0x642c, 0, 0x01);
	radeon_mask(rdev, 0x642c, 0xff << 16, 0);
	radeon_mask(rdev, 0x6430, 0, 0x01);
	radeon_mask(rdev, 0x6430, 0xff << 16, 0);
	udelay(50);
...
static ssize_t kalindi_dp_aux_transfer(struct drm_dp_aux *aux,
...
struct drm_dp_aux my_aux = {
	.transfer = kalindi_dp_aux_transfer,
	.i2c_nack_count = 0,
	.i2c_defer_count = 0,
	.parent = &my_i2c
};
